...
def paint(event):	# 이전 점과 현재 점 사이를 직선으로 연결한다. 
    ...
    if   old_x and old_y and mode != "rect":
...
def reset(event):	# 사용자가 마우스 버튼에서 손을 떼면 이전 점을 삭제한다. 
    global old_x, old_y, var, mycolor
    if mode == "rect":
        print("rr")
        canvas.create_rectangle(old_x, old_y, event.x, event.y, width=var.get(), outline=mycolor)
...
def use_rect():	# 캔버스에 그려진 모든 그림을 삭제한다. 
    global mode
    mode = "rect"  
    
def rect(event):	# 캔버스에 그려진 모든 그림을 삭제한다. 
...
clearButton = Button(window, text='사각형', command=use_rect)
clearButton.grid(row=0, column=5, sticky=W+E)
